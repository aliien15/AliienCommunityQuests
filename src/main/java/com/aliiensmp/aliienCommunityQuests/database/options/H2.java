...
public class H2 implements DatabaseProvider {
    ...
    public H2(final AliienCommunityQuests plugin, final String prefix) {
        this.plugin = plugin;
        this.prefix = prefix;
    }
    ...
    public CompletableFuture<Void> saveActiveQuest(final String questId, final Map<String, Integer> objectiveProgress, final Set<UUID> participants, final long endTime) {
        String updateProgress = "MERGE INTO " + prefix + "active_quest_objectives (quest_id, objective_id, progress, end_time) KEY(quest_id, objective_id) VALUES (?, ?, ?, ?);";
        String insertParticipant = "MERGE INTO " + prefix + "active_quest_participants (quest_id, player_uuid) KEY(quest_id, player_uuid) VALUES (?, ?);";
